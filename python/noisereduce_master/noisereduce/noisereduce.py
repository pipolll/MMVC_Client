...
class SpectralGateStationary(SpectralGate):
    def __init__(
        ...
        # ここnumpyにしてる
        temp = librosa.stft(
                (np.asnumpy(self.y_noise)),
                n_fft=self._n_fft,
                hop_length=self._hop_length,
                win_length=self._win_length,
            )
        abs_noise_stft = np.asarray(temp)
        noise_stft_db = _amp_to_db(np.asnumpy(abs_noise_stft)) 
    ...
    def spectral_gating_stationary(self, chunk):
        ...
        for ci, channel in enumerate(chunk):
            ...
            sig_stft_db = _amp_to_db(np.asnumpy(np.abs(np.asarray(sig_stft))))
            sig_stft_db = np.asarray(sig_stft_db)
            ...
            sig_mask = sig_stft_db > np.asarray(db_thresh)
            ...
            # multiply signal with mask
            sig_stft_denoised = np.asarray(sig_stft) * sig_mask

            # invert/recover the signal
            denoised_signal = librosa.istft(
                np.asnumpy(sig_stft_denoised),
                hop_length=self._hop_length,
                win_length=self._win_length,
            )
            denoised_signal = sig_mask = np.asarray(denoised_signal)
            denoised_channels[ci, : len(denoised_signal)] = denoised_signal
        return denoised_channels
